...
globals [lu lu1 one two three four five six sevenone seventwo eight nine suitab s1 a i j k l n any-centroids-moved? update_res_count totalres_patches ] 
turtles-own [lu_t ]
...
;  ]
  ...
  ask patches [
    if (lu_p = 1)   [set plabel 1]   ; printing labels
    ]
 
   ask one-of patches with [plabel = 1]
   [set pcolor green]
   
   
   ask patches [
   if one-of neighbors = 1
   [xpand]
   ]
   set a patches with [pcolor = green]
   print a
...
to residential_new
  ...
  ask one-of patches with  [(lu_p = 29) or (lu_p = 26) and (suitab_p > 4)  ]
...
to tryres
  ...
  ask one-of neighbors with [(lu_p = 1) or (lu_p = 3)])
  [
end
