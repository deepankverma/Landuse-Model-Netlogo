...
to setup-patches
  ask patches
  [
    set attraction ( random max-attraction )   ; asking patches to randomize attraction value
  ]
  repeat smoothness
  [
    diffuse attraction .4   ; to diffuse the value to fellow neighbors.
  ]
  ask patches
  [
    set pcolor scale-color green attraction 2.5 10  ; accordingly setting color gradient.
  ]
end
...
to setup-turtles
  ...
  [
   ...
   ask turtles [ move-to one-of patches with [attraction > 2.5]  ]  ; asking the population variable to move to the patches with attraction greater than 2.5.
end
